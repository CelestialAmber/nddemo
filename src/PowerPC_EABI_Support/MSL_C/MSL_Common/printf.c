...
static int __pformatter(void* (*WriteProc)(void*, const char*, size_t), void* WriteProcArg, const char* format_str, va_list arg)
{
	...
	while (*format_ptr) {
		...
		case 'g':
		case 'G':
			if (!(buff_ptr = float2str(arg, buff + 512, &format, vecIndex))) {
				goto conversion_error;
			}
...
{
	return (fwrite(pBuffer, 1, char_num, (FILE*)file) == char_num ? file : 0);
}
...
int vprintf(const char* pFormat, va_list arg)
{
	...
	return __pformatter(&__FileWrite, (void*)stdout, pFormat, arg);
}
